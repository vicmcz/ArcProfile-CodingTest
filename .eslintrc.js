...
module.exports = {
  root: true,
  extends: ['prettier', '@arcblock/eslint-config'],
  globals: {
    logger: true,
  },
  rules: {
    ...
    quotes: [2, 'single'], // 单引号
    'no-console': [1, { allow: ['error'] }], // 禁用console
    ...
    'no-unused-vars': [isDev ? 1 : 2, { vars: 'all', args: 'after-used' }], // 不能有声明后未被使用的变量或参数
    ...
    'require-await': 2, // 禁止使用不带 await 表达式的 async 函数 (require-await)
    'no-shadow': 0,
    'no-else-return': 0,
    'no-return-await': 0,
